...
var vertx = Vertx.vertx(VertxOptions(
  metricsOptions = VertxMonitoringOptions(
...
router.route("/metrics").handler({ routingContext ->
  var prometheusRegistry = io.vertx.monitoring.backend.BackendRegistries.getDefaultNow()
...
You can interact with `link:../../apidocs/io/vertx/monitoring/VertxMonitoringOptions.html#disabledMetricsCategories[disabledMetricsCategories]` to restrict the
...
// Later on:
var registry = io.vertx.monitoring.backend.BackendRegistries.getDefaultNow()
...
// Later on:
var registry = io.vertx.monitoring.backend.BackendRegistries.getNow("my registry")
...
Note: all metrics here may receive a "remote" label, but it is deactivated by default. See `link:../../apidocs/io/vertx/monitoring/VertxMonitoringOptions.html#setEnableRemoteLabelForServers-boolean-[setEnableRemoteLabelForServers]`
