...
router.route("/metrics").handler(function (routingContext) {
  var prometheusRegistry = Java.type("io.vertx.monitoring.backend.BackendRegistries").getDefaultNow();
...
You can interact with `link:todo[disabledMetricsCategories]` to restrict the
...
// Later on:
var registry = Java.type("io.vertx.monitoring.backend.BackendRegistries").getDefaultNow();
...
// Later on:
var registry = Java.type("io.vertx.monitoring.backend.BackendRegistries").getNow("my registry");
...
Note: all metrics here may receive a "remote" label, but it is deactivated by default. See `link:../dataobjects.html#VertxMonitoringOptions#setEnableRemoteLabelForServers[enableRemoteLabelForServers]`
