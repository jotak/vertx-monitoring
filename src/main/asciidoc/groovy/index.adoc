...
----
compile 'io.vertx:vertx-micrometer-metrics:3.5.2'
...
A word about naming: the different metric backends may have different conventions or rules for naming metrics.
For instance, Influx convention is to use dot as a word separator, whereas Prometheus uses underscore.
The names described below are given with underscore separators, but the actual metric names may vary
 according to the backend that is used.
